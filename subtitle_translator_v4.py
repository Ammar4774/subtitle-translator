...
class SubtitleTranslatorApp:
    def __init__(self, root):
        ...
        self.subtitle_overlay = tk.Frame(self.video_frame, bg="", highlightthickness=0)
    ...
    def show_translation_box(self, translation, label_widget):
        ...
        # Ensure translation box is raised above other widgets
        self.translation_box.lift()
        # Place relative to video frame to avoid overlap with subtitle overlay
        x = label_widget.winfo_rootx() - self.video_frame.winfo_rootx()
        y = label_widget.winfo_rooty() - self.video_frame.winfo_rooty() - self.translation_box.winfo_reqheight() - 10
        self.translation_box.place(x=x, y=y, anchor="sw")
        self.video_frame.update_idletasks()  # Force GUI update
        self.translation_box_hide_job = self.root.after(3000, self.hide_translation_box)

    def hide_translation_box(self):
        self.translation_box.place_forget()
        self.translation_box.config(text="")
        self.video_frame.update_idletasks()  # Force GUI update
        self.root.update()  # Ensure full refresh
        self.translation_box_hide_job = None
    ...
    def run_ffmpeg_extract(self, video_file, track_idx):
        try:
            ...
            cmd = [
                "ffmpeg", "-y", "-i", video_file, "-map", f"0:s:{track_idx}", srt_path
            ]
            result = subprocess.run(cmd, stderr=subprocess.PIPE, stdout=subprocess.PIPE, text=True)

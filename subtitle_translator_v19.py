...
class SubtitleTranslatorApp:
    ...
    def handle_word_click(self, word, label_widget):
        ...
        self.show_translation_box("Translating...", label_widget)

        def translate_and_update():
            translation = self.translate_word(word)
            self.root.after(0, self.update_translation_box, translation, label_widget)
    ...
    def show_translation_box(self, text, label_widget):
        self.safe_print(f"Showing translation box for: {text}")
        self.translation_box = tk.Label(self.root, text=text, font=("Segoe UI", 18, "italic"), bg="#fffbe6", fg="#333", bd=2, relief="ridge", padx=16, pady=8)
    ...
    def update_translation_box(self, translation, label_widget):
        if self.translation_box:

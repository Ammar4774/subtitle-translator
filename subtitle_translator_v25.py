...
class SubtitleTranslatorApp:
    ...
    def parse_srt_file(self, srt_path):
        ...
        for match in pattern.finditer(content):
            ...
            text = text.replace('\n', ' ').strip()
            subtitles.append({'start': start_sec, 'end': end_sec, 'text': text, 'index': int(idx) - 1})
        return subtitles
    ...
    def update_subtitles(self, event=None):
        ...
        if subtitle_line:
            ...
            if current_subtitle and current_subtitle['index'] + 1 < len(self.subtitles):
                next_subtitle = self.subtitles[current_subtitle['index'] + 1]
                self.cache_next_subtitle(next_subtitle)
        else:
    ...
    def handle_word_click(self, word, label_widget):
        ...
        translation = self.translation_cache.get(word, self.translate_word(word))
    ...
    def translate_word(self, word):
        ...
        try:
            ...
            self.translation_cache[word] = translated_text
    ...
    def cache_next_subtitle(self, subtitle):
        def translation_worker():
            words = re.findall(r'\b\w+\b', subtitle['text'].lower())
            for word in words:

...
class SubtitleTranslatorApp:
    def __init__(self, root):
        ...
        self.translation_frame = tk.Frame(self.root, bg="#e0e0e0", pady=5)
        self.translation_frame.pack(fill=tk.X, side=tk.BOTTOM)
        self.original_word_label = tk.Label(self.translation_frame, text="", font=("Segoe UI", 12), bg="#e0e0e0")
        self.original_word_label.pack(side=tk.LEFT, padx=10)
        self.translated_word_label = tk.Label(self.translation_frame, text="", font=("Segoe UI", 12, "bold"), bg="#e0e0e0")
        self.translated_word_label.pack(side=tk.LEFT, padx=10)
        ...
        self.player_controls_frame.pack(pady=5, side=tk.BOTTOM)
        ...
        self.load_controls_frame.pack(pady=5, side=tk.BOTTOM)
    ...
    def pre_translate_subtitles(self):
        unique_words = set()
        for sub in self.subtitles:
            words = re.findall(r'\b\w+\b', sub['text'].lower())
            unique_words.update(words)

        self.status_label.config(text=f"Pre-translating {len(unique_words)} unique words...")
        self.root.update_idletasks()

        def translation_worker():
            for word in unique_words:
                if word not in self.translation_cache:
                    self.translation_cache[word] = self.translate_word(word)
            self.root.after(0, self.on_pre_translation_complete)

        threading.Thread(target=translation_worker, daemon=True).start()

    def on_pre_translation_complete(self):
        self.status_label.config(text="Pre-translation complete. Ready.")
    ...
    def handle_word_hover(self, word, label_widget):
        ...
        translation = self.translation_cache.get(word, "Translating...")
        self.update_translation_display(word, translation)
    ...
    def update_translation_display(self, original, translated):
        self.original_word_label.config(text=f"Original: {original}")
        self.translated_word_label.config(text=f"Translation: {translated}")

...
class SubtitleTranslatorApp:
    def __init__(self, root):
        ...
        self.subtitle_track_var = tk.StringVar(self.root)
        self.subtitle_track_var.set("No Subtitles") # default value
        self.subtitle_track_menu = tk.OptionMenu(self.load_controls_frame, self.subtitle_track_var, "No Subtitles")
        self.subtitle_track_menu.grid(row=0, column=3, padx=5)
        self.subtitle_track_var.trace("w", self.change_subtitle_track)
        ...
        self.translation_box = None # Will be created dynamically
    ...
    def load_subtitle_tracks(self):
        ...
        # Clear existing options in the menu
        self.subtitle_track_menu["menu"].delete(0, "end")
        self.subtitle_track_menu["menu"].add_command(label="No Subtitles", command=tk._setit(self.subtitle_track_var, "No Subtitles"))
        self.subtitle_track_var.set("No Subtitles")

        if not tracks:
            messagebox.showinfo("Info", "No embedded subtitle tracks found.")
            return

        for track_id, track_name in tracks:
            track_name_decoded = track_name.decode('utf-8', errors='ignore')
            self.subtitle_tracks.append((track_id, track_name_decoded))
            self.subtitle_track_menu["menu"].add_command(label=track_name_decoded, command=tk._setit(self.subtitle_track_var, track_name_decoded))

        # Auto-select Spanish if available
        spanish_track_name = None
        for track_id, track_name in self.subtitle_tracks:
            if "spanish" in track_name.lower() or "español" in track_name.lower() or "es" in track_name.lower():
                spanish_track_name = track_name
                break

        if spanish_track_name:
            self.subtitle_track_var.set(spanish_track_name)
            self.status_label.config(text=f"Selected subtitle track: {spanish_track_name}")
        else:
            self.status_label.config(text="Select a subtitle track from the dropdown.")

    def change_subtitle_track(self, *args):
        selected_track_name = self.subtitle_track_var.get()
        if selected_track_name == "No Subtitles":
            self.player.video_set_spu(-1) # Disable subtitles
            self.status_label.config(text="Subtitles disabled.")
            self.subtitles = [] # Clear loaded subtitles
        else:
            track_id = None
            for tid, tname in self.subtitle_tracks:
                if tname == selected_track_name:
                    track_id = tid
                    break
            if track_id is not None:
                self.player.video_set_spu(track_id)
                self.status_label.config(text=f"Selected subtitle track: {selected_track_name}")
                # For embedded subtitles, VLC handles the display. We don't parse SRT here.
                self.subtitles = [] # Clear any previously loaded external SRT
            else:
                messagebox.showerror("Error", f"Could not find track: {selected_track_name}")
    ...
    def load_subtitles(self):
        ...
        if srt_file:
            ...
            self.player.video_set_spu(-1) # Disable embedded subtitles if external SRT is loaded
            self.subtitle_track_var.set("No Subtitles") # Reset dropdown
        else:
    ...
    def update_subtitles(self, event=None):
        ...
        subtitle_line = ""
        # Only display custom parsed subtitles if no embedded track is selected
        if not self.player.video_get_spu() > -1 and self.subtitles:
        ...
        if subtitle_line:
            ...
            for word in words:
                ...
                lbl.bind("<Enter>", lambda e, w=clean_word, l=lbl: self.handle_word_hover(w, l))
                lbl.bind("<Leave>", lambda e, l=lbl: self.hide_translation_box())
        ...
        # Hide translation box if no subtitle is displayed
        if not subtitle_line and self.translation_box and self.translation_box.winfo_ismapped():
            self.hide_translation_box()

    def handle_word_hover(self, word, label_widget):
        self.safe_print(f"Word hovered: {word}")
        # Pause video on word hover
        if self.player.is_playing():
            self.player.pause()
            self.play_pause_btn.config(text="Play")
    ...
    def hide_translation_box(self):
        if self.translation_box:
            self.translation_box.destroy()
            self.translation_box = None
        # Ensure the area is redrawn
        self.subtitle_overlay.update_idletasks()
